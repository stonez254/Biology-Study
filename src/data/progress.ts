...
export type StudyProgress = {
  ...
  lessonReadDate: string | null; ratRetakeDate: string | null;
};
...
const defaultProgress: StudyProgress={points:0,streak:0,lastStudyDate:null,attempts:[],missedQuestionIds:[],revisionAttempts:[],lessonReadDate:null,ratRetakeDate:null};
...
export function getProgress():StudyProgress{const raw=read<Partial<StudyProgress>>(PROGRESS_KEY,defaultProgress);return{
  ...
  lessonReadDate:raw.lessonReadDate??null,ratRetakeDate:raw.ratRetakeDate??null
};}
...
export function markLessonRead():StudyProgress{const progress=getProgress();const next={...progress,lessonReadDate:localDateKey()};saveProgress(next);return next;}
export function hasReadLessonToday(progress=getProgress()){return progress.lessonReadDate===localDateKey();}
